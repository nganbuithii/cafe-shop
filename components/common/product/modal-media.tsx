...
import { Keyboard, Navigation, Pagination, Zoom } from "swiper"
...
interface HTMLElement {
  requestFullscreen?: () => void // Chrome
  webkitRequestFullscreen?: () => void // Safari
  mozRequestFullScreen?: () => void // Firefox
  msRequestFullscreen?: () => void // IE/Edge
}

interface Document {
  exitFullscreen?: () => void // Chrome
  webkitExitFullscreen?: () => void // Safari
  mozCancelFullScreen?: () => void // Firefox
  msExitFullscreen?: () => void // IE/Edge
}
...
const ModalMedia = (props: Props) => {
  ...
  const handleOpenFullScreen = () => {
    const elem = document.getElementById("mediaFullScreen") as HTMLElement
    if (!isFullScreen) {
      if (elem) {
  ...
  const handleChangeSlide = (swiper: any) => {
    ...
    const mediaVideo = document.getElementById("mediaVideo") as HTMLMediaElement
    if (mediaVideo) {
      mediaVideo["play"]()
    }
  }
  ...
  return (
    <Modal className="w-full h-full z-[999999]" classNameContainer="h-full w-full" status={props.status} closedRawer={handleCloseModal} fullWidth={true}>
        ...
        <div className="flex flex-col justify-between h-full w-full" id="mediaFullScreen">
          <div className="h-full w-full select-none flex flex-col items-center relative">
            <div className="h-[42px] w-full bg-black relative flex items-center justify-between lg:px-[32px] pr-[18px] pl-[17px]">
              <span className="text-white text-[15px] leading-[1.5] lg:block hidden">
                {props.activeIndex + 1}/{props.mediaList.length}
              </span>
              <div className="flex items-center lg:w-auto lg:justify-start lg:flex-row w-full justify-between flex-row-reverse">
                <div className="flex items-center lg:mr-[88.5px] mr-0">
                  {props.mediaList[props.activeIndex]?.media_type === MEDIA_TYPE.IMAGE && (
                    <>
                      <div className="px-[11.5px] cursor-pointer" onClick={handleZoomOut}>
                        <Image
                          src={zoomOut ? "/images/icons/ic-zoom-out-disabled.svg" : "/images/icons/ic-zoom-out.svg"}
                          width={26}
                          height={26}
                          objectFit="contain"
                      ...
                      <div className="px-[11.5px] cursor-pointer" onClick={handleZoomIn}>
                        <Image
                          src={zoomIn ? "/images/icons/ic-zoom-in-disabled.svg" : "/images/icons/ic-zoom-in.svg"}
                          width={26}
                          height={26}
                          objectFit="contain"
                          className="lg:h-[26px] lg:w-[26px] w-[24px] h-[24px]"
                  ...
                  <div className="px-[11.5px] cursor-pointer" onClick={handleOpenFullScreen}>
                    <Image src={"/images/icons/ic-scan.svg"} width={20} height={20} objectFit="contain" className="h-[20px] w-[20px]" alt="icon" />
                  </div>
                </div>
                <div className="cursor-pointer" onClick={() => handleCloseModal()}>
                  <Image src={isMobile ? "/images/icons/ic-prev-white.svg" : "/images/icons/ic-close-white.svg"} width={26} height={26} objectFit="cover" alt="icon" />
                </div>
              </div>
            </div>
            <div className="media-swiper relative w-full">
              <div className="absolute top-1/2 left-[83px] -translate-y-1/2 cursor-pointer drop-shadow-navigation swiper-navigation-prev-1 z-[999] lg:flex hidden">
                <Image src="/images/icons/ic-prev-circle.svg" width={37} height={37} objectFit="cover" alt="prevIcon" />
              </div>
              <Swiper
                ref={swiperRef}
                slidesPerView={1}
                    ...
                    <SwiperSlide className="flex items-center justify-center" key={index} onClick={handleClickSwiperSlide}>
                      <div className="relative w-full h-full flex items-center justify-center">
                        {item.media_type === MEDIA_TYPE.IMAGE ? (
                          <Image src={replaceUrlImage(item?.url)} objectFit="contain" width={0} height={0} layout="responsive" className="media-swiper-item" alt={"mediaImage"} />
                        ) : (
                          <video autoPlay={false} muted={muted} playsInline controls id={"mediaVideo"} className="media-swiper-item">
              ...
              <div className="absolute top-1/2 right-[83px] -translate-y-1/2 cursor-pointer drop-shadow-navigation swiper-navigation-next-1 z-[999] lg:flex hidden">
                <Image src="/images/icons/ic-next-circle.svg" width={37} height={37} objectFit="cover" alt="prevIcon" />
              </div>
            </div>
          </div>
        </div>
