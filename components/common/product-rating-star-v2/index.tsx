...
const StarList = (props: Props) => {
  const renderStarArray = (total: number) => {
    ...
    Array.from({ length: total }, (_, i) => {
      arr.push(<IoStar key={i} color={"#fcdf3a"} className="StarPurple500" size={14} />)
    })
    if (total - Math.floor(total) > 0.3) {
      arr.push(<IoStarHalf color={"#000"} className="StarPurple500" size={14} />)
    }
    Array.from(
      {
        length: Math.ceil(5 - (total - Math.floor(total) > 0.3 ? Math.floor(total) + 1 : Math.floor(total)))
      },
      (_, i) => arr.push(<IoStarOutline key={i} color={"#000"} className="StarPurple500" size={14} />)
    )
    return arr
  }
  return (
