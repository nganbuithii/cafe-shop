...
const ReviewProduct = () => {
  ...
  const [storeReview, actionReview] = useProductReview() as any
      ...
      return images.map((image, index: number) => {
        return (
          <div className="relative w-[90px] h-[90px] border border-black rounded-common my-[10px] mr-[15px]" key={index}>
            <Image src={URL.createObjectURL(image) || ""} layout="fill" objectFit="cover" className="rounded-common overflow-hidden" alt="reviewImage" />
            <FiXCircle className="cursor-pointer absolute top-[-8px] right-[-8px] bg-white rounded-full z-[2]" onClick={() => actionReview.handleRemoveImage(index)} size={20} />
          </div>
        )
      })
    }
        ...
        <div className="lg:mx-[149px] lg:px-[42px] bg-white">
          ...
          <div className="grid grid-cols-[80px_1fr] items-center lg:mt-0 mt-[30px]">
            <div className="overflow-hidden rounded-[10px] bg-product border flex">
              <Image
                src={replaceUrlImage(storeProduct.product?.image_base_url || storeProduct.product?.product_media?.[0]?.url || "")}
                width={80}
                height={80}
                objectFit="cover"
          ...
          <form className="mt-[41px] w-full" onSubmit={handleSubmit(onSubmit)}>
            <textarea id="feedback" {...register("feedback")} cols={30} className={styleInput} placeholder={t.opinionOnTheProductZiliCoffee} rows={10} />
              ...
              <div className="flex items-center flex-wrap px-[10px] rounded-[10px] bg-media-review mt-[20px]">
                {storeReview.bodyReview?.video && (
                  <div className="relative w-[90px] h-[90px] border border-black rounded-common my-[10px] mr-[15px]">
                    <video src={URL.createObjectURL(storeReview.bodyReview?.video)} muted autoPlay className="rounded-common overflow-hidden w-full h-full object-cover" />
                    <FiXCircle className="cursor-pointer absolute top-[-8px] right-[-8px] bg-white rounded-full z-[2]" onClick={() => actionReview.setVideo(null)} size={20} />
                  </div>
                )}
                {!storeReview.bodyReview?.video && (
                  <div className="relative w-[90px] h-[90px] my-[10px]">
                    ...
                    <Dropzone accept={{ "video/mp4": [".mp4", ".MP4"] }} maxFiles={LIMIT_VIDEO} onDrop={(files) => actionReview.setVideo(files[0])}>
                      {({ getRootProps, getInputProps }) => (
                        <div {...getRootProps({ className: "w-full h-full absolute top-0 left-0" })}>
                          <input {...getInputProps()} />
                        </div>
            ...
            <div className="mt-[20px]">
              ...
              {PRODUCT_FLAVORS?.length &&
                PRODUCT_FLAVORS.map((flavor: any, index: number) => {
              ...
              {PRODUCT_TYPES?.length &&
                PRODUCT_TYPES.map((productType: any, index: number) => {
            ...
            <div className="lg:mt-[87px] mt-[48px] lg:pb-[49px] pb-[24px] flex items-center justify-center">
              <button className="btn-primary capitalize text-primary rounded-[10px] py-[13px] lg:w-[41.1596958175%] w-full" disabled={storeReview.isLoading} type="submit">
                {t.submitReview}
              </button>
            </div>
          </form>
