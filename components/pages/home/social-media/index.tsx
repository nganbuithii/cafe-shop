  ...
  return (
    <Link href={props.linkUrl || ""} target="_blank">
      <a className="block lg:mx-[17.5px] mx-[11px]">
        <Image src={props.icon || ""} objectFit="cover" priority={true} alt="media" height="23px" width="23px" />
      </a>
    </Link>
  )
}
...
const items = [
  {
    ...
    description:
      "Cà Phê Đóng Chai Wowi Coffee sẵn sàng phục vụ bạn những lúc cần thêm caffein khi làm việc tại nhà hay chạy deadline với hội nhóm.Bật chế độ tỉnh táo với 3 sự lựa chọn: Cà Phê Đen, Cà Phê Sữa, Coldbrew--",
    product: {
      name: "ROBUSTA ZILI",
      price: 220000,
...
const SocialMedia = () => {
  ...
  return (
    <>
                ...
                return (
                  ...
                  >
                    <div className="relative w-full h-0 pt-[100%]">
                      <Image src={item.image_url || ""} layout="fill" objectFit="cover" priority={true} alt="socialImage" />
                      {item.type === MEDIA_TYPE.VIDEO && (
                        <div className="absolute w-[37px] h-[37px] top-[50%] left-[50%] -translate-x-1/2 -translate-y-1/2">
                          <div className="relative w-full h-0 pt-[100%]">
                            <Image src="/images/icons/ic-action.svg" layout="fill" objectFit="cover" priority={true} alt="socialImage" />
                          </div>
                        </div>
                      )}
                    </div>
      ...
      <ModalDetail status={isShowModal} onCloseModal={handleCloseModal} items={items} initialIndex={activeIndex} />
    </>
  )
}
