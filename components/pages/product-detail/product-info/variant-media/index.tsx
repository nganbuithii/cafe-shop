          ...
          <>
            <div className="variant-media-swiper__big relative w-full">
              <div className="absolute top-1/2 left-[12px] lg:w-[37px] lg:h-[37px] w-[25px] h-[25px] rounded-full -translate-y-1/2 cursor-pointer shadow-navigation swiper-navigation-prev-2 z-[999] flex">
                <Image src="/images/icons/ic-prev-circle.svg" layout="fill" objectFit="cover" alt="prevIcon" />
              </div>
              <Swiper
              ...
              >
                {storeProduct.product.product_media.map((item, index) => {
                  ...
                  return (
                    <SwiperSlide key={index} className="p-[3px]">
                      {item.media_type === MEDIA_TYPE.IMAGE && (
                        <div className="relative productImageJs w-full h-0 pt-[100%]" onClick={() => handleMediaSelected(index)}>
                          <Image src={replaceMediaUrl} objectFit="cover" priority={true} layout="fill" alt="mediaImage" />
                        </div>
                      )}
                      {item.media_type === MEDIA_TYPE.VIDEO && (
                        <>
                          ...
                          {(swiperIndexActive !== index || item.url_thumb) && (
                            <div className="relative productImageJs lg:h-full h-auto lg:pt-0 pt-[100%]" onClick={() => handleMediaSelected(index)}>
                              <Image src={replaceMediaUrl} objectFit="cover" priority={true} layout="fill" alt="mediaImage" />
                            </div>
                          )}
                        </>
              ...
              <div className="absolute top-1/2 right-[12px] lg:w-[37px] lg:h-[37px] w-[25px] h-[25px] rounded-full -translate-y-1/2 cursor-pointer shadow-navigation swiper-navigation-next-2 z-[999] flex">
                <Image src="/images/icons/ic-next-circle.svg" layout="fill" objectFit="cover" alt="prevIcon" />
              </div>
            </div>
            <div className="variant-media-swiper__small w-full">
              <Swiper onSwiper={(swiper) => setThumbsSwiper(swiper)} spaceBetween={8} slidesPerView="auto" watchSlidesProgress watchOverflow modules={[Thumbs]}>
                {storeProduct.product.product_media.map((item, index) => {
                    ...
                    <SwiperSlide key={index} className="p-[1px]">
                      <div className="overflow-hidden pt-[100%] relative h-0 w-full bg-product-image">
                        ...
                        {item.media_type === MEDIA_TYPE.VIDEO && !item.url_thumb && (
                          <video playsInline loop id={storeProduct.product?.id} muted={true} className="object-cover absolute top-0 left-0 right-0 bottom-0 w-full h-full" src={replaceMediaUrl} />
                        )}
                        {item.media_type === MEDIA_TYPE.VIDEO && (
                          <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2">
                            <Image src="/images/icons/ic-play-video.svg" objectFit="cover" priority={true} width={24} height={24} alt="playIcon" />
                          </div>
                        )}
                      </div>
                    </SwiperSlide>
