    ...
    <>
      <div className="lg:mt-[45px] mt-[16px]">
        <div className="flex items-center w-full">
          <StarDropdown selectedStar={storeReview.filter.totalStar} handleSelectStarFilter={actionReview.handleSelectStarFilter} />
          <TypeDropdown selectedType={storeReview.filter.type} handleSelectTypeFilter={actionReview.handleSelectTypeFilter} />
        </div>
        <div className="lg:mt-[30px] mt-[16px]">
          {storeReview.reviewsByProduct?.reviews &&
            storeReview.reviewsByProduct.reviews.length > 0 &&
            storeReview.reviewsByProduct.reviews.map((review, index) => {
              return <ReviewItem item={review} handleSelectedMedia={handleSelectedMedia} key={index} />
            })}
          {(!storeReview.reviewsByProduct?.reviews || storeReview.reviewsByProduct?.reviews?.length === 0) && <NoData title={"noReviewYet"} imageUrl="/images/img-no-review.webp" />}
        </div>
